...
@Component({
  selector: 'app-monkuse',
  templateUrl: './monkuse.component.html',
  styleUrls: ['./monkuse.component.scss']
})
export class MonkuseComponent implements OnInit {
  ...
  getData() {
    const firebase = collection(this.firestore, 'monkuse');
    getDocs(firebase).then((response) => {
      this.monkuseList = [...response.docs.map((item) => {
